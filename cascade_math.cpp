...
inline rect 
RectLeftTopWidthHeight(r32 Left, r32 Top, r32 Width, r32 Height)
{
    rect Result;
...
HsvaToRgba(v4 Color) 
{
    float C = Color.Val * Color.Sat;
    float X = C * (1.0 - Abs_u32((u32)(Color.Hue / 60.0) % 2 - 1));

    if (Color.Hue >= 300) {
        return V4(C, 0.0, X, Color.a);
    } else if (Color.Hue >= 240) {
        return V4(X, 0.0, C, Color.a);
    } else if (Color.Hue >= 180) {
        return V4(0.0, X, C, Color.a);
    } else if (Color.Hue >= 120) {
        return V4(0.0, C, X, Color.a);
    } else if (Color.Hue >= 60) {
        return V4(X, C, 0.0, Color.a);
    }
    
    return V4(C, X, 0.0, Color.a);
}
